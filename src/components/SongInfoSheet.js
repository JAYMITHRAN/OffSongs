...
export default function SongInfoSheet({ visible, song, onClose }) {
  if (!song) return null;

  const st = statsFor(song.id);
  const ext = (song.uri || '').split('.').pop()?.toUpperCase() || 'AUDIO';
  ...
  return (
    <Sheet visible={visible} title="Song Details" onClose={onClose}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.section}>
          ...
          <InfoRow icon="folder-outline" label="Folder" value={song.folder || 'Music'} />
